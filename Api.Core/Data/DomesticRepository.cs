...
{
    public class DomesticRepository
    {
        #region Postage
        public string Cuoc_TN(int Mabc_Nhan, int Mabc_Tra, int Khoi_Luong, int Vung_Xa)
        {
            OracleCommand cmd = new OracleCommand("CUOC_WEB.EmsDomestic_TN");
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add(new OracleParameter("v_MabcNhan", OracleDbType.Int32)).Value = Mabc_Nhan;
            cmd.Parameters.Add(new OracleParameter("v_MabcTra", OracleDbType.Int32)).Value = Mabc_Tra;
            cmd.Parameters.Add(new OracleParameter("v_khoiluong", OracleDbType.Int32)).Value = Khoi_Luong;
            cmd.Parameters.Add(new OracleParameter("v_edi", OracleDbType.Int32)).Value = Vung_Xa;
            cmd.Parameters.Add(new OracleParameter("v_cuoc", OracleDbType.Int32)).Direction = ParameterDirection.Output;
            OracleDataReader dr = Helper.ExecuteDataReader(cmd, Helper.OraPtemsOracleConnection);
            return cmd.Parameters["v_cuoc"].Value.ToString();

        }
        public string Cuoc_TN_TT(int Mabc_Nhan, int Mabc_Tra, int Khoi_Luong)
        {
            OracleCommand cmd = new OracleCommand("CUOC_WEB.EmsDomesticAgree");
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add(new OracleParameter("v_MaTinhNhan", OracleDbType.Int32)).Value = Mabc_Nhan;
            cmd.Parameters.Add(new OracleParameter("v_MaTinhTra", OracleDbType.Int32)).Value = Mabc_Tra;
            cmd.Parameters.Add(new OracleParameter("v_Khoi_Luong", OracleDbType.Int32)).Value = Khoi_Luong;
            cmd.Parameters.Add(new OracleParameter("v_Cuoc", OracleDbType.Int32)).Direction = ParameterDirection.Output;
            OracleDataReader dr = Helper.ExecuteDataReader(cmd, Helper.OraPtemsOracleConnection);
            return cmd.Parameters["v_cuoc"].ToString();
        }
        public string Cuoc_QT(string Nuoc_Tra, int Khoi_Luong, int Loai)
        {
            OracleCommand cmd = new OracleCommand("CUOC_WEB.EmsInternational_VND");
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add(new OracleParameter("v_manctra", OracleDbType.Varchar2)).Value = Nuoc_Tra;
            cmd.Parameters.Add(new OracleParameter("v_khoiluong", OracleDbType.Int32)).Value = Khoi_Luong;
            cmd.Parameters.Add(new OracleParameter("v_PhanLoai", OracleDbType.Int32)).Value = Loai;
            cmd.Parameters.Add(new OracleParameter("v_Cuoc", OracleDbType.BinaryFloat)).Direction = ParameterDirection.Output;
            OracleDataReader dr = Helper.ExecuteDataReader(cmd, Helper.OraPtemsOracleConnection);
            return cmd.Parameters["v_cuoc"].Value.ToString(); ;
        }
        #endregion
        ...
        public List<Delivery> TrackingProcessF(string code)
        {
            code = code.Trim().ToUpper();
            DataTable COD_TABLE0 = new DataTable();
            DataTable COD_TABLE1 = new DataTable();
        ...
        #region Transfer_infomation đóng chuyển
        public List<Transfer_Info> Transfer_infomation(string code)
        {
            EmsRepository _ems = new EmsRepository();
            Transfer_Info _transfer = null;
            List<Transfer_Info> _listTransfer_infomation = null;
            DataTable dt1 = new DataTable();
            DataTable dt2 = new DataTable();
            int m_type = 0;
            _ems.Put_Pod_To_TMP_By_E1(code, ref m_type);
            dt1 = _ems.GetDeliveryByE1(code);
            dt2 = _ems.GetDeliveryByE1_STC(code);

            _listTransfer_infomation = new List<Transfer_Info>();
            if (dt1.Rows.Count > 0)
            {
                for (int j = 0; j < dt1.Rows.Count; j++)
                {
                    _transfer = new Transfer_Info();
                    _transfer.Date = Convert_Date(dt1.Rows[j]["NGAY"]);
                    _transfer.Time = Convert_Time(dt1.Rows[j]["GIO"]);
                    _transfer.Receive_Po = Convert.ToString(dt1.Rows[j]["MABC"]) + "-" + GET_TEN_BC_BY_MA_BC(dt1.Rows[j]["MABC"].ToString());
                    _transfer.bag = Convert.ToInt32(dt1.Rows[j]["TUISO"]);
                    _transfer.trip = Convert.ToInt32(dt1.Rows[j]["CHTHU"]);
                    _transfer.Trend = (string)dt1.Rows[j]["HUONG"];
                    _listTransfer_infomation.Add(_transfer);
                }
            }
            if (dt2.Rows.Count > 0)
            {
                ...
                {
                    _transfer = new Transfer_Info();
                    _transfer.Date = (string)dt2.Rows[j]["NGAY"];
                    _transfer.Time = (string)dt2.Rows[j]["GIO"];
                    _transfer.Receive_Po = dt2.Rows[j]["MABC"] + "-" + GET_TEN_BC_BY_MA_BC((string)dt2.Rows[j]["MABC"]);
                    _transfer.bag = Convert.ToInt32(dt2.Rows[j]["TUI"]);
                    _transfer.trip = Convert.ToInt32(dt2.Rows[j]["CHTHU"]);
                    _transfer.Trend = (string)dt2.Rows[j]["HUONG"];
                    _listTransfer_infomation.Add(_transfer);
                }
            }

            return _listTransfer_infomation;
        }
        #endregion

        #region E1 infomation
        public Lading E1_infomation(string code)
        {
            DataSet ds = new DataSet();
            EmsRepository _ems = new EmsRepository();
            Lading _lading = new Lading();
            _lading = _ems.getThongTinE1(code);
        ...
        #region Multi tracking
        public DataTable BindMultiTracking(string code)
        {
            EmsRepository _ems = new EmsRepository();
            string strE1 = "";
            string strTrongNuoc = "";
            ...
            int t = 1;
            DataTable myDataSet;
            DataTable myDataSetQT;
            strE1 = code.Replace(" ", "");

            if (CheckStr(strE1) == false)
            {
                strE1 = "";
                //Message.Visible = true;
                //Message.Text = "Chu?i nh?p vào quá dài.";
            }
            else
            {

            }
            string[] splitStr = Regex.Split(strE1, "\n");
            foreach (string str in splitStr)
            {
                _ems.Put_Pod_To_TMP_By_E1(str, ref intCheck);
                if (intCheck == 1) // Hàng Qu?c t?
                {
                    strQuocTe += "'" + str + "'" + ", ";
                }
                else
                {
                    strTrongNuoc += "'" + str + "'" + ", ";
                    _ems.TTS_DELETIVERY(str);
                }
            }
            if (strTrongNuoc.Length != 0)
            {
                strTrongNuoc = strTrongNuoc.Remove(strTrongNuoc.Length - 2, 2);
            }
            if (strQuocTe.Length != 0)
            {
                strQuocTe = strQuocTe.Remove(strQuocTe.Length - 2, 2);
            }
            if (strTrongNuoc.Length > 13)
            {
                myDataSet = _ems.GetDeliveryByE1MultiData(strTrongNuoc);
                for (int i = 0; i < myDataSet.Rows.Count; i++)
                {
                    if (i >= 1)
                    {
                        if (myDataSet.Rows[i]["MaE1"].ToString() == myDataSet.Rows[i - p]["MaE1"].ToString())
                        {
                            myDataSet.Rows[i]["MaE1"] = " ";
                            p += 1;
                        }
                        else
                        {
            ...
            if (strQuocTe.Length > 13)
            {
                _ems.Get_Pod_From_TMP_By_MultiE1(strQuocTe, 0);
                myDataSetQT = _ems.Get_Pod_From_TMP_By_MultiE1(strQuocTe, 0);
                if (myDataSetQT.Rows.Count > 0)
                {

                }
                for (int i = 0; i < myDataSetQT.Rows.Count; i++)
                {
                    if (i >= 1)
                    {
                        if (myDataSetQT.Rows[i]["MaE1"].ToString() == myDataSetQT.Rows[i - t]["MaE1"].ToString())
                        {
                            myDataSetQT.Rows[i]["MaE1"] = " ";
                            t += 1;
                        }
                        else
                        {
        ...
        private Boolean CheckStr(string strE1)
        {
            int n = 0;
            n = strE1.Length;
            if (n > 139)
        ...
        public ResponseDelivery ListItemStatus(string trackingcode)
        {
            ResponseDelivery response = new ResponseDelivery();
            List<Delivery> lstDelivery;
            Delivery delivery = null;
            var code = trackingcode.ToUpper().Trim();
            try
            {
                using (OracleCommand cm = new OracleCommand())
                {
                    ...
                    if(checkcode(code))
                        cm.CommandText = string.Format("SELECT * FROM BCCP_TRACESTATUS P WHERE P.ITEMCODE= '" + code + "' ORDER BY DELIVERYTIME DESC");
                    else
                        cm.CommandText = string.Format("SELECT * FROM BCCP_TRACESTATUS P WHERE P.ITEMCODE=(Select MAE1 FROM E1E2_PH WHERE SO_THAM_CHIEU= '" + code + "' AND ROWNUM =1) ORDER BY DELIVERYTIME DESC");
                    cm.CommandType = CommandType.Text;
                    OracleDataReader dr = Helper.ExecuteDataReader(cm, Helper.OraDCOracleConnection);
                    if(dr.HasRows)
                    {
                      lstDelivery = new List<Delivery>(); 
                        while (dr.Read())
                        {
                            delivery = new Delivery();
                            delivery.Date = dr["DELIVERYTIME"].ToString();
                            delivery.Status = dr["CAUSECODE"].ToString();
                            delivery.Note = dr["DELIVERYNOTE"].ToString();
                            delivery.Location = "POST CODE: " + dr["TOPOSCODE"].ToString();
                            delivery.Cause  = dr["CAUSENAME"].ToString();
                            delivery.Solution = dr["SOLUTIONNAME"].ToString();
                            delivery.ReceiverName = dr["RECEIVERNAME"].ToString();
                            lstDelivery.Add(delivery);
                        }
                        response.code = 200;
                        response.message = "Success";
                        response.ListDelivery = lstDelivery;
                    }
            ...
            {
                LogAPI.LogToFile(LogFileType.EXCEPTION, "Domestic.ListItemStatus: " + trackingcode + "." + ex.Message);
                response.code = 501;
